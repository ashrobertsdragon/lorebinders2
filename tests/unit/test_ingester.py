...
@pytest.fixture
def ingester():
    return EbookIngester()


def test_split_chapters_with_asterisks(ingester):
    """Test splitting text by '***' delimiter."""
    text = "Chapter 1\nSome text.\n***\nChapter 2\nMore text."
    chapters = ingester._split_chapters(text)
...
def test_split_chapters_with_markdown_headers(ingester):
...
def test_split_chapters_fallback(ingester):
...
def test_ingest_flow(ingester, mock_ebook2text, tmp_path):
...
def test_ingest_file_not_found(ingester, tmp_path):
    ...
    with pytest.raises(FileNotFoundError):
        ingester.ingest(Path("nonexistent.epub"), tmp_path)


def test_empty_chapter_filter(ingester):
