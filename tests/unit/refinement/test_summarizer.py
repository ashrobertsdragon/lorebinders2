...
from lorebinders.agents.summarization import EntitySummarizationAgent, summarization_agent
...
def test_summarization_agent_run_sync_and_prompt():
    """Test EntitySummarizationAgent run_sync execution and prompt generation."""
    ...
    with summarization_agent.override(model=FunctionModel(mock_model_call)):
        agent = EntitySummarizationAgent()
...
def test_entity_summarizer_orchestration():
    """Test that EntitySummarizer correctly constructs config and orchestrates the agent."""
    ...
    with summarization_agent.override(model=FunctionModel(mock_model_call)):
        ...
        result_binder = summarizer.summarize(binder)
