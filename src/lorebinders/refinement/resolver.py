...
class EntityResolver:
    """Merges duplicate entities and combines their data."""
    ...
    def _is_similar_key(self, key1: str, key2: str) -> bool:
        ...
        detitled_k1 = self.remove_titles(k1)
        detitled_k2 = self.remove_titles(k2)
        singular_k1 = self.to_singular(k1)
        singular_k2 = self.to_singular(k2)
        ...
        k1_is_title = k1 in self.TITLES
        k2_is_title = k2 in self.TITLES
        if (k1_is_title and k1 + " " in k2) or (k2_is_title and k2 + " " in k1):
            return True

        if (detitled_k1 + " " in detitled_k2) or (
            detitled_k2 + " " in detitled_k1
        ):
            return True
    ...
    def _prioritize_keys(self, key1: str, key2: str) -> tuple[str, str]:
        ...
        if (l1 in l2 or l2 in l1) and l1 != l2:
            if l1 in self.TITLES:
                return key2, key1
            if l2 in self.TITLES:
                return key1, key2
    ...
    def resolve(self, binder: dict[str, Any]) -> dict[str, Any]:
        ...
        for category, entities in binder.items():
            ...
            for n1, n2 in combinations(names, 2):
                ...
                if self._is_similar_key(n1, n2):
                    to_merge, to_keep = self._prioritize_keys(n1, n2)
                    working_entities[to_keep] = self._merge_values(
